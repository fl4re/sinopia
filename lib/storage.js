...
Storage.prototype.get_package = function(name, options, callback) {
  ...
  self.local.get_package(name, options, function(err, data) {
    ...
    self._sync_package_with_uplinks(name, data, options, function(err, result, uplink_errors) {
      if (err) return callback(err)
      var whitelist = [ '_rev', 'name', 'versions', 'dist-tags', 'readme', 'time' ]
      ...
      for (var i in result) {
        ...
        for (var j = 0; j < Object.keys(result.versions).length; j++) {
          var version = Object.keys(result.versions)[j]
          var info = result.versions[version]
          result.time[version] = info.time || current_time
        }
        if (!result.time.modified) {
          result.time.modified = current_time
        }
      }
    ...
    self.local.get_package(locals[i], function(err, info) {
      if (!err) {
        ...
        if (latest && info.versions[latest]) {
          var version = info.versions[latest]
          var time = info.time[latest]
          version.time = time
