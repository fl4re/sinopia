...
Storage.prototype.add_version = function(name, version, metadata, tag, callback) {
  ...
  self.update_package(name, function updater(data, cb) {
    ...
    // some old storage do not have 'time' field
    // this replaces lodash isNil method
    var is_nil = data.time === null || data.time === NaN || data.time === void 0;
    if (is_nil) {
      data.time = {};
    }
      ...
      if (stats.mtime > startkey) {
        self.get_package(item.name, options, function(err, data) {
          ...
          versions.forEach(function (version) {
            time[version] = item.versions[version] && item.versions[version].time && item.versions[version].time.modified;
          });
